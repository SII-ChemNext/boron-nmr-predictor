...
# 1. Configuration
# =============================================================================
MODEL_NAME = "BoronNMRNet_V3_ML_Fusion"
LOG_FILENAME = "logs/training_v3_ml_fusion_log-2_reproduce.txt"

# Hyperparameters - Trial 39 (Best performing trial with MAE: 2.1711)
BATCH_SIZE = 16
...
# ML global feature configuration (SHAP Top-20)
ML_FEATURE_DIM = 20    # Top-20 SHAP features
ML_HIDDEN_DIM = 64     # Encoded dimension of ML features
...
def compute_sorted_loss(all_preds, all_targets, batch_indices):
    """
    Node-level loss aggregation: weighted by number of boron atoms,
    ensuring training objective is consistent with evaluation metrics.
    """
    ...
    for mol_idx in unique_mols:
        ...
        # reduction='sum' gives molecule-level total loss
...
def train_one_epoch(model, loader, optimizer):
    ...
    for batch in loader:
        ...
        # Get ML global features (if available)
...
def print_experiment_header(train_len, test_len, total_len):
    ...
    print(f"   Description     : 5-Fold CV with Virtual Node + Solvent Embedding + ML Feature Fusion")
    ...
    print(f"   ML Feature Dim  : {ML_FEATURE_DIM}")
    print(f"   ML Hidden Dim   : {ML_HIDDEN_DIM}")
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dev_dataset_list)):
        ...
        # Initialize model (with ML global feature fusion)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            if val_mae < best_res['mae']:
                ...
                torch.save(model.state_dict(), f"{MODEL_DIR}/model_v3_fold_{fold+1}.pth")
    ...
    for fold in range(K_FOLDS):
        ...
        try:
            m.load_state_dict(torch.load(f"{MODEL_DIR}/model_v3_fold_{fold+1}.pth",
