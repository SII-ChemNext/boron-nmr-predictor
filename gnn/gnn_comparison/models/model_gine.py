...
from torch_geometric.nn import GINEConv, BatchNorm, global_add_pool

class BoronNMRNet_GINE(nn.Module):
    """
    GINE-based 11B NMR prediction model (with ML Feature Top-20 concatenation)

    Features:
    1. GINEConv (Graph Isomorphism Network with Edge features)
    2. Virtual Node mechanism
    3. Learnable solvent embedding (32-dim)
    4. Theoretically most expressive (WL test equivalent)
    ...
    def __init__(self, node_in_dim, edge_in_dim,
        ...
        # ========================================
        # 2. GNN backbone (GINEConv + virtual node)
        # ========================================
        # GINE requires an MLP as the neural network
        mlp1 = nn.Sequential(
        ...
        self.conv1 = GINEConv(mlp1, edge_dim=hidden_dim, train_eps=True)
        ...
        self.conv2 = GINEConv(mlp2, edge_dim=hidden_dim, train_eps=True)
        ...
        self.conv3 = GINEConv(mlp3, edge_dim=hidden_dim, train_eps=True)
